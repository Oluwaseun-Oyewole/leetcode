...
export default function App() {
  async function handleSubmit(event) {
    ...
    if (formObject.method === "post") {
      if (
        formData.get("name") &&
        formData.get("email") &&
        formData.get("message")
      ) {
        const response = await fetch(formObject.action, {
        ...
        alert(text);
      } else alert("Fill yout form fields");
    } else alert("Incorrect form method");
  }
  return (
    <form
    ...
    >
      ...
      <textarea col={20} row={20} name="message" />
      <button>Send</button>
    </form>
  );
}
